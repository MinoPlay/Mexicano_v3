...
- **Notification ELO drifting from a separate recompute.** `sendTournamentCompletedPush()` used to
  recompute ELO by replaying the full match history from scratch
  (`computeTournamentEloChanges`), a different code path than the baseline-chained value
  `finalizeCompletedTournament()` writes to `Store.getPlayersSummary()`. If the local match
  cache was incomplete, the two diverged and the push showed the wrong ELO. Fix: prefer the
  already-computed `players_summary` (elo/previousElo), falling back to the replay only for a
  player missing from the summary.
- **ELO looking wrong until a few refreshes.** `players.json` on GitHub is regenerated by a
  server-side pipeline retriggered by the tournaments.json push (see `pushCompletedTournament`).
  A pull that lands before that pipeline finishes can fetch pre-tournament ELO and regress the
  locally-correct value. Fix: `github.js` `applyEloBaselineOverlay()` overlays the local
  `mexicano_elo_baseline` snapshot (written by `finalizeCompletedTournament`) onto a freshly
  pulled summary when it belongs to the latest complete tournament date.

...
- The title `#home-title` renders `🎾 Mexicano v<APP_VERSION>` (plus ` · preview:<slug>` on a branch preview — see `.github/features/preview-deployments.md`) and is clickable. After confirmation, it removes `matches`, `matches_fully_loaded`, `active_tournament`, and `completion_marker`, then reloads the page.

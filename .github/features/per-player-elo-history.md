...
- Player line/dot colors are **linked to entry number** (0-based selection order), not the name. Fixed color table `ELO_ENTRY_COLORS` holds 10 prefixed colors; the first 5 are hue-wise very different (≥45° apart) so top selections never look similar. Past 10 entries colors are generated via golden-angle hue spread. `buildEntryColorMap(orderedNames)` maps first-seen selection order → color; map is rebuilt on every selection change.

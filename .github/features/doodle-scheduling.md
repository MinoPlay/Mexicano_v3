...
- **Player Overview**: collapsible `<details>` section below "Overall availability". Shows actual tournament participation for the current month. Players sourced from `Store.getMonthlyOverview(ym)` (players_overview.json); falls back to match participants. Tournament dates (columns) derived from `Store.getMatches()` filtered to month. ✓ where player actually played. ID column shows `MatchPadelId` from players_summary; for `MatchPadelId === 0` players the ID cell shows `playedCount × 90kr` cost instead. 💰 emoji shown inline next to name for players without a MatchPadel account. Date columns collapsible via "◀/▶ Dates" toggle button (`.dates-hidden` CSS class).

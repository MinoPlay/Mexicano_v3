...
- **Save popup**: centered modal overlay appears when user has unsaved changes (date toggled). Contains "Save Changes?" heading + Save (primary) + Cancel (ghost) buttons. Save disabled + shows "Saving…" during async save; re-enables on failure. Dismissed on successful save or cancel.

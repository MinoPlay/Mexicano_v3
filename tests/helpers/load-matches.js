...
// Frozen snapshot boundary. The ELO parity fixtures (elo-expected.json,
// monthly-elo-expected.json) were generated by the C# source-of-truth from the
// backup-data state up to and including this date (6452 matches, 66 players).
// backup-data is append-only, so pinning the cutoff makes the ELO parity suite
// deterministic: matches/players added to live data after this date can never
// leak into the tests and shift the expected values. Do not bump this without
// regenerating the fixtures from the C# generator against the same cutoff.
const FROZEN_CUTOFF = '2026-05-12';
      ...
      if (statSync(full).isDirectory()) {
        ...
        if (data.matches) {
          for (const m of data.matches) {
            if (m.Date > FROZEN_CUTOFF) continue;
            allMatches.push({
              date: m.Date,
